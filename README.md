...
The `make contract` command is the key CI gate: it regenerates the OpenAPI which is in `/frontend/.openapi.json` spec from the Laravel codebase, regenerates the TypeScript types from that spec, then runs `tsc --noEmit` to verify the frontend still compiles against the latest contract.
